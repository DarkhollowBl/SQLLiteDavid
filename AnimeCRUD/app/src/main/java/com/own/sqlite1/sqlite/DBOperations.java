...
public final class DBOperations {
    ...
    /* Method of song*/
    //ANIME METHOD MADE LIRA FIERRO
    //Operations of song
    public Cursor getAnimes(){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s", DBHelper.Tables.ANIME);
        return  database.rawQuery(sql, null);
    }

    public Cursor getAnimeById(String id){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s" +
                        " WHERE %s=?",
                DBHelper.Tables.ANIME,
                Contract.Animes.ID);
        String[] whereArgs = {id};
        return database.rawQuery(sql, whereArgs);
    }

    public String insertAnime(Anime anime){
        SQLiteDatabase database = db.getWritableDatabase();
        String idSong = Contract.Animes.generateIdAnime();
        ContentValues values = new ContentValues();
        values.put(Contract.Animes.ID, idSong);
        values.put(Contract.Animes.NAME, anime.getName());
        values.put(Contract.Animes.CREATOR, anime.getCreator());
        values.put(Contract.Animes.GENDER, anime.getGender());
        values.put(Contract.Animes.YEAR, anime.getYear());
        values.put(Contract.Animes.CHAPTERS, anime.getChapters());
        database.insertOrThrow(DBHelper.Tables.ANIME, null, values);
        return idSong;
    }
    ...
    public boolean updateAnime(Anime anime){
        ...
        values.put(Contract.Animes.YEAR, anime.getYear());
        values.put(Contract.Animes.CHAPTERS, anime.getChapters());
        String whereClause = String.format("%s=?", Contract.Animes.ID);
        String[] whereArgs = {anime.getIdAnime()};
        int result = database.update(DBHelper.Tables.ANIME, values, whereClause, whereArgs);
        return result>0;
    }

    public boolean deleteAnime(String idSong) {
        SQLiteDatabase database = db.getWritableDatabase();
        String whereClause = Contract.Animes.ID + "=?";
        String[] whereArgs = {idSong};
        int result = database.delete(DBHelper.Tables.ANIME, whereClause, whereArgs);
        return result > 0;
    }
